...
namespace Script
{
    ...
    public class CompareExcel : BaseSystemExcel
    {
        public CompareExcel()
        {
            ...
            var listTables = ExcelByBase.Data.ImportToDataTable(dir1, false).Select(p => (ListTable) p).ToDictionary(p => p.TableName);
            var listTables2 = ExcelByBase.Data.ImportToDataTable(dir2, false).Select(p => (ListTable) p).ToDictionary(p => p.TableName);
